...
function String2Pascal( S: String ): String;
...
begin
  ...
  begin
    ...
    begin
      if Result <> '' then
        Result := Result + ' + ';
      Result := Result + String2PascalStrExpr( Copy( S, 1, 255 ) );
      S := Copy( S, 256, MaxInt );
    end;
  end
    else
...
begin
  ...
  try
    ...
    Rpt( '*/\* EXCEPTION - Could not remove current selection' );
  end;
end;
...
begin
  ...
  begin
    ...
    if C <> 0 then
    begin
      Rpt( '~~~ color not found: ' + Int2Hex( C, 6 ) );
      Exit;
    end;
  end;
  Result := TRUE;
...
begin
  ...
  TRY
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            Result := KOL.pf24bit;
            Rpt( '~~~~~ pf24bit (break) ~~~~~ (' + Int2Str( ColorList.Count ) + ')' );
            Exit;
          end;
        end;
        Inc( L );
    ...
    //Rpt( '------ Colors in bitmap: ' + Int2Str( ColorList.Count ) );
    ...
    begin
      Result := KOL.pf24bit;
      Rpt( '~~~~~ pf24bit ~~~~~ (' + Int2Str( ColorList.Count ) + ')' );
    end;
  FINALLY
    ColorList.Free;
  END;
...
begin
  ...
  {ShellExecute( 0, 'open', PChar( ExtractFilePath( Application.ExeName ) + 'brcc32.exe' ),
                ...
                SW_HIDE );}
  Rpt( 'Compiling resource ' + ProjectSourcePath + FileName + '.rc' );
  ...
  begin
    Rpt( 'Can not compile resource with ' + ExtractFilePath( Application.ExeName ) + 'brcc32.exe' );
  end;
  if FileExists( S ) then
  begin
    J := FileSize( S );
...
begin
  ...
  if hFR = INVALID_HANDLE_VALUE then begin
      Rpt( 'Can not create file ' + Res + #13#10'Error: ' + SysErrorMessage( GetLastError ) );
      Exit;
  end;
...
begin
  if Multimedia then Result := 'TMMTimer'
...
begin
  ...
  begin
    ...
    //ShowMessage( 'Bitmap.Handle=' + Int2Str( Bitmap.Handle ) );
    FKOLImgList.Clear;
    ...
    begin
      ...
      //ShowMessage( 'Result := ' + Int2Str( R ) );
      //ShowMessage( 'FKOLImgList.Handle=' + Int2Str( FKOLImgList.Handle ) );
    end;
  end;
end;
...
procedure TKOLImageList.SetupLast(SL: TStringList; const AName, AParent,
  Prefix: String);
begin
  asm
    jmp @@e_signature
    DB '#$signature$#', 0
    DB 'TKOLImageList.SetupLast', 0
  @@e_signature:
  end;
  inherited;
end;
...
begin
  ...
  GenerateTag( SL, AName, Prefix );
  if InitialPath <> '' then
    SL.Add( Prefix + '  ' + AName + '.InitialPath := ' + StringConstant( 'InitialDir', InitialPath )
            + ';' );
