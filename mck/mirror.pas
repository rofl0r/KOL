...
//////////////////////////////////////////////////////////
  ...
  private
    ...
    function ConvertVCL2KOL( ConfirmOK: Boolean ): Boolean;
...
//    procedure Loaded; override;
  ...
  private
    ...
    FgenearteSepeartorConstants: Boolean;
    ...
    procedure SetgenearteSepeartorConstants(const Value: Boolean);
  ...
  published
    ...
    property genearteSepeartorConstants: Boolean read FgenearteSepeartorConstants write SetgenearteSepeartorConstants;
...
{$IFDEF MCKLOG}
var EnterLevel: Integer;
    LevelOKStack: array[ -1000..+1000 ] of Boolean;
{$ENDIF MCKLOG}
procedure Log( const S: String );
begin
  {$IFDEF MCKLOG}
  ...
  begin
    Inc( EnterLevel );
    if (EnterLevel >= -1000) and (EnterLevel <= 1000) then
      LevelOKStack[ EnterLevel ] := FALSE;
  end
  else
  if Copy( S, 1, 2 ) = '<-' then
  begin
    if (EnterLevel >= -1000) and (EnterLevel <= 1000) then
      if not LevelOKStack[ EnterLevel ] then
        LogFileOutput( 'C:\MCK.log', DateTime2StrShort( Now ) + ' ' +
                       IntToStr( EnterLevel ) + ' *** Leave not OK *** ' + S );
    Dec( EnterLevel );
  end;
  {$IFDEF MCKLOGwoRPT}
  ...
  LogFileOutput( 'C:\MCK.log', DateTime2StrShort( Now ) + ' ' + IntToStr( EnterLevel ) + ' ' + S );
  {$ENDIF MCKLOG}
end;
...
begin
  ...
  TRY
    ...
    if (Left <> R.Left) or (Top <> R.Top) or
      ...
      ReAlign( FALSE );
    //Log( 'TKOLCustomControl.SetBounds6 (before Change)' );
    Change;
...
begin
  ...
  Log( '->TKOLCustomControl.SetParent' );
  try
  ...
  begin
    ...
    begin
      ...
      begin
        KF := ParentKOLForm;
        FFont.Assign( KF.Font );
      end
        else
      begin
        KC := Value as TKOLCustomControl;
        FFont.Assign( KC.Font );
      end;}
    end;
    //Font.Assign(RunTimeFont); {YS}
    //Log( '1 - Get_ParentFont' );
    PF := Get_ParentFont;
    //Log( '2 - Get_ParentFont' );
    Font.Assign(PF); {YS}
    //Log( '3 - Get_ParentFont' );
  end;
{YS}
  {$IFDEF _KOLCtrlWrapper_}
  //Log( '1 - PaintType' );
  PT := PaintType;
  //Log( '2 - PaintType - AllowSelfPaint' );
  FAllowSelfPaint := PT in [ptWYSIWIG, ptWYSIWIGFrames];
  //Log( '3 - AllowSelfPaint - AllowCustomPaint' );
  FAllowCustomPaint:=PT <> ptWYSIWIG;
  //Log( '4 - AllowCustomPaint' );
  {$ENDIF}
{YS}
  //Log( '5 - Change, Self=$' + Int2Hex( DWORD( Self ), 6 ) );
  CodeAddr := Change;
  //Log( '6 - Change Addr: $' + Int2Hex( DWORD( TMethod( CodeAddr ).Code ), 6 ) );
  TRY
    Change;
...
begin
  ...
  //if (BFont = nil) or (BFont = Font) or not Font.Equal2( BFont ) then
  if not Font.Equal2( PFont ) then
...
begin
  ...
  if (Width <> DefaultWidth) or (Height <> DefaultHeight) then
  begin
    if (Width <> DefaultWidth) and not (Align in [ caTop, caBottom ]) then
      W := Width;
    if (Height <> DefaultHeight) and not (Align in [ caLeft, caRight ]) then
      H := Height;
  end;
  ...
  if IsGenerateSize then
  if not (autoSize and AutoSizeRunTime) or fNoAutoSizeX then
  begin
...
begin
  ...
  if fChangingNow or ( csLoading in ComponentState ) or (Name = '') then
  begin
    LogOK; Exit;
  end;
...
{YS}
  {if not MinimizeIcon and not MaximizeIcon then
    SL.Add( Prefix + AName + '.Style := ' + AName + '.Style and not (WS_MINIMIZEBOX or WS_MAXIMIZEBOX);' )
  else
  begin
    if not MinimizeIcon then
      SL.Add( Prefix + AName + '.Style := ' + AName + '.Style and not WS_MINIMIZEBOX;' );
    if not MaximizeIcon then
      SL.Add( Prefix + AName + '.Style := ' + AName + '.Style and not WS_MAXIMIZEBOX;' );
  end;}
  {if not CloseIcon then
    SL.Add( Prefix + AName + '.ClsStyle := ' + AName + '.ClsStyle or CS_NOCLOSE;' );}

  if Transparent then
    SL.Add( Prefix + AName + '.Transparent := True;' );

  //Log( '&9 TKOLForm.SetupFirst' );

  if (AlphaBlend <> 255) and (AlphaBlend > 0) then
    SL.Add( Prefix + AName + '.AlphaBlend := ' + IntToStr( AlphaBlend and $FF ) + ';' );

  //Log( '&010 TKOLForm.SetupFirst' );

  if not HasBorder then
    SL.Add( Prefix + AName + '.HasBorder := False;' );

  //Log( '&011 TKOLForm.SetupFirst' );

  if not HasCaption and HasBorder then
    SL.Add( Prefix + AName + '.HasCaption := False;' );

  //Log( '&012 TKOLForm.SetupFirst' );

  if StayOnTop then
    SL.Add( Prefix + AName + '.StayOnTop := True;' );

  //Log( '&013 TKOLForm.SetupFirst' );

  if not Ctl3D then
    SL.Add( Prefix + AName + '.Ctl3D := False;' );

  //Log( '&014 TKOLForm.SetupFirst' );
  ...
  //Log( '&017 TKOLForm.SetupFirst' );

  {if Color <> clBtnFace then
    SL.Add( Prefix + AName + '.Color := ' + Color2Str( Color ) + ' ;' );}
  if Brush <> nil then
    Brush.GenerateCode( SL, AName );

  //Log( '&018 TKOLForm.SetupFirst' );

  if (Font <> nil) AND not Font.Equal2( nil ) then
    Font.GenerateCode( SL, AName, nil );

  //Log( '&019 TKOLForm.SetupFirst' );

  if Border <> 2 then
    SL.Add( Prefix + AName + '.Border := ' + IntToStr( Border ) + ';' );

  //Log( '&020 TKOLForm.SetupFirst' );

  if MarginTop <> 0 then
    SL.Add( Prefix + AName + '.MarginTop := ' + IntToStr( MarginTop ) + ';' );

  //Log( '&021 TKOLForm.SetupFirst' );

  if MarginBottom <> 0 then
    SL.Add( Prefix + AName + '.MarginBottom := ' + IntToStr( MarginBottom ) + ';' );

  //Log( '&022 TKOLForm.SetupFirst' );

  if MarginLeft <> 0 then
    SL.Add( Prefix + AName + '.MarginLeft := ' + IntToStr( MarginLeft ) + ';' );

  //Log( '&023 TKOLForm.SetupFirst' );

  if MarginRight <> 0 then
    SL.Add( Prefix + AName + '.MarginRight := ' + IntToStr( MarginRight ) + ';' );

  //Log( '&024 TKOLForm.SetupFirst' );
  ...
  //Log( '&026 TKOLForm.SetupFirst' );

  AssignEvents( SL, AName );

  //Log( '&027 TKOLForm.SetupFirst' );

  if EraseBackground then
    SL.Add( Prefix + AName + '.EraseBackground := TRUE;' );

  //Log( '&028 TKOLForm.SetupFirst' );

  if MinWidth > 0 then
    SL.Add( Prefix + AName + '.MinWidth := ' + IntToStr( MinWidth ) + ';' );

  //Log( '&029 TKOLForm.SetupFirst' );

  if MinHeight > 0 then
    SL.Add( Prefix + AName + '.MinHeight := ' + IntToStr( MinHeight ) + ';' );

  //Log( '&030 TKOLForm.SetupFirst' );

  if MaxWidth > 0 then
    SL.Add( Prefix + AName + '.MaxWidth := ' + IntToStr( MaxWidth ) + ';' );

  //Log( '&031 TKOLForm.SetupFirst' );

  if MaxHeight > 0 then
    SL.Add( Prefix + AName + '.MaxHeight := ' + IntToStr( MaxHeight ) + ';' );

  //Log( '&032 TKOLForm.SetupFirst' );
...
function TKOLProject.ConvertVCL2KOL( ConfirmOK: Boolean ): Boolean;
var I, E, N: Integer;
    F: TKolForm;
    S: String;
begin
  ...
  begin
    ...
    begin
      F := FormsList[ I ];
      if not F.FChanged then continue;
...
begin
  ...
  Result := FALSE;
  if KOLProject <> nil then
    Result := KOLProject.ConvertVCL2KOL( FALSE );
  if not Result then
  begin
...
var N: Integer;
  ...
  begin
    ...
    begin
      if not MI.separator or genearteSepeartorConstants then
...
procedure TKOLMenu.SetgenearteSepeartorConstants(const Value: Boolean);
begin
  FgenearteSepeartorConstants := Value;
  Change;
end;
