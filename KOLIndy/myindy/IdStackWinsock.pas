...
begin
  ...
  begin
    result := Int2Str(Ord(s_b1)) + '.' + Int2Str(Ord(s_b2)) + '.' +
      Int2Str(Ord(s_b3)) + '.'
      + Int2Str(Ord(s_b4));
  end;
end;
...
begin
  with TInAddr(AInAddr).S_un_b do
  begin
    s_b1 := Chr(Str2Int(Fetch(AIP, '.')));
    s_b2 := Chr(Str2Int(Fetch(AIP, '.')));
    s_b3 := Chr(Str2Int(Fetch(AIP, '.')));
    s_b4 := Chr(Str2Int(Fetch(AIP, '.')));
  end;
end;
