...
type
  TIdUDPBase = object(TIdComponent)
  protected
...
PIdUDPBase=^TIdUDPBase;
function NewIdUDPBase(AOwner: PControl):PIdUDPBase;{ type  MyStupid0=DWord;
  EIdUDPException = object(EIdException);
PdUDPBase=^IdUDPBase; type  MyStupid3137=DWord;
  EIdUDPReceiveErrorZeroBytes = object(EIdUDPException);
PUDPBase=^dUDPBase; type  MyStupid86104=DWord;        }

implementation

uses
  IdResourceStrings, IdStack, IdStackConsts;
...
function NewIdUDPBase(AOwner: PControl):PIdUDPBase;
begin
//  inherited;
  New( Result, Create );
with Result^ do
