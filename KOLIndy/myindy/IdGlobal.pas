...
begin
  ...
  Result := Format('%s, %d %s %d %s %s', {do not localize}
    ...
    wYear, Time2StrFmt('hh:mm:ss', Value), {do not localize}
...
var
  sFile: PStream;
begin
  sFile := NewReadFileStream(sFilename);
  try
    result := sFile.Size;
  finally
    sFile.free;
...
begin
  DateTime2SystemTime(Value, SysTimeVar);
...
{              raise EIdCorruptServicesFile.CreateFmt(RSCorruptServicesFile,
          ...
          i := Str2Int(Copy(s, i + 1, iPosSlash - i - 1));
          if i <> iPrev then
          begin
...
begin
  ...
  begin
    ...
    begin
      try
        Result := EncodeTime(Str2Int(Copy(s, 2, 2)), Str2Int(Copy(s, 4, 2)),
          0, 0);
...
begin
  ...
  while StrReplace(psSrc, '+', ' ') do; {do not localize}
  i := 1;
  ...
  begin
    ...
    begin
      ...
      Inc(i, 1);
      try
        CharCode := Str2Int('$' + ESC); {do not localize}
...
begin
  ...
  begin
    ...
    begin
      Result := Result + '%' + Int2Hex(Ord(psSrc[i]), 2); {do not localize}
    end
    else
    begin
      Result := Result + psSrc[i];
