...
begin
  ...
  begin
    ...
    begin
      Fetch(RangeDecode);
      FContentRangeStart := Str2Int(Fetch(RangeDecode, '-'));
      FContentRangeEnd := Str2Int(Fetch(RangeDecode, '/'));
    end;
...
begin
  ...
  begin
    ...
    if FContentLength <> 0 then
      Add('Content-Length: ' + Int2Str(FContentLength)); {do not localize}
    if (FContentRangeStart <> 0) or (FContentRangeEnd <> 0) then
    begin
      if FContentRangeEnd <> 0 then
        Add('Range: bytes=' + Int2Str(FContentRangeStart) + '-' +
          Int2Str(FContentRangeEnd))
      else
        Add('Range: bytes=' + Int2Str(FContentRangeStart) + '-');
    end;
    if FUsername <> '' then
...
{        if not (Intercept is TIdSSLConnectionIntercept) then
  ...
  begin
    ...
    begin
      ...
      begin
        if Pos('chunked', Response.ExtraHeaders.Values['Transfer-Encoding'])
...
{    if ASource is TStrings then
  ...
  Fetch(FResponseText);
  FResponseCode := Str2Int(Fetch(FResponseText, ' ', False));
...
begin
  ...
  sPort := Int2Str(FTunnelProxyPort);
