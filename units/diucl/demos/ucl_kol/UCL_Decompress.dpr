{ ------------------------------------------------------------------------------
  ...
  UCL_Decompress -- Example using a UCL Decompression stream with KOL
...
begin
  if ParamCount < 2 then
    begin
      WriteLn('Usage: UCL_Decompress.exe UCL-Compressed-File OutFile');
  ...
  try
    if Source.Size = STREAM_ERROR then Exit;
    Dest := NewWriteFileStream(ParamStr(2));
    try
      if Dest.Size = STREAM_ERROR then Exit;
      if Source.Read(Size, 4) <> 4 then Exit;
      if not NewUclDDStream(UclStream, $40000, Source, nil) then Exit;
      ...
      Error := not (StreamCopy(Dest, UclStream, Size) = Size);
      UclStream.Free;
    finally
      Dest.Free;
      if Error then
