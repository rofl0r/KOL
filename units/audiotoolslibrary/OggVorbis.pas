...
begin
  ...
  try
    ...
    Source.Seek(Info.ID3v2Size, spBegin);
    Source.Read(Info.FPage, SizeOf(Info.FPage));
    if Info.FPage.ID <> OGG_PAGE_ID then exit;
    Source.Seek(Info.ID3v2Size + Info.FPage.Segments + 27, spBegin);
    ...
    Source.Seek(Info.SPagePos + Info.SPage.Segments + 27, spBegin);
...
function RebuildFile(FileName: string; Tag: PStream; Info: FileInfo): Boolean;
var
...
procedure TOggVorbis.InitFields;
begin
  { Object constructor }
  FResetData;
end;
...
begin
  ...
  begin
    ...
    if FTrack > 0 then Info.Tag.FieldData[4] := Int2Str(FTrack)
