...
// [+] - добавил свойство ReadOldVersion = False, иначе заглатывает все подряд.
...
type
  { Class TMPEGplus }
    ...
    public
      ...
      property ReadOldVersion: Boolean read FReadOldVersion write FReadOldVersion;
  end;
...
begin
  try
    ...
    Transferred:= SourceFile.Read(Header, 12);
...
procedure TMPEGplus.InitFields;
begin
  FID3v1 := NewID3v1;
  FID3v2 := NewID3v2;
  FAPEtag := NewAPE;
  FReadOldVersion:= False;
  FResetData;
end;
...
begin
  ...
  if not FReadOldVersion then Result:= GetStreamVersion(Header)=7;
