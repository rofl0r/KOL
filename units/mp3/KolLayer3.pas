...
uses
  Windows, Kol, KolShared, KolBitReserve,
  KolBitStream, KolHeader, KolSynthFilter,
  KolL3Type, kolobuffer_mci;
...
type
  ...
  PLayerIII_Decoder = ^TlayerIII_decoder;
  TLayerIII_Decoder = object(Tobj)
  private
    ...
    FStream: PBitStream;
    FHeader: PHeader;
    FFilter1, FFilter2: PSynthesisFilter;
    FBuffer: POBuffer_mci;
  ...
  public
    //    constructor Create(Stream: PBitStream; Header: PHeader; FilterA, FilterB: PSynthesisFilter;
    //                       Buffer: POBuffer; Which_Ch: TChannels);
...
function NewLayerIII_Decoder(Stream: PBitStream; Header: PHeader;
  FilterA, FilterB: PSynthesisFilter; Buffer: POBuffer_mci;
  Which_Ch: TChannels): PLayerIII_Decoder;

implementation

uses
  KolHuffman, KolMath, KolInvMDT, KolL3Tables;

{ TLayerIII_Decoder }

procedure TLayerIII_Decoder.Antialias(ch, gr: cardinal);
var 
  ss, sb18, sb18lim: cardinal;
  gr_info: PGRInfo;
  bu, bd: single;
...
begin
  ...
  if (gr_info.window_switching_flag <> 0) and (gr_info.block_type = 2)
    and (gr_info.mixed_block_flag = 0) then
    exit;

  if (gr_info.window_switching_flag <> 0) and (gr_info.mixed_block_flag <> 0)
    and (gr_info.block_type = 2) then
    sb18lim := 18
  else
    sb18lim := 558;
...
begin
  ...
  begin
    ...
    if (gr_info.window_switching_flag <> 0) and
      (((gr_info.block_type = 2) and (gr_info.mixed_block_flag = 0)) or
...
const
  NrOfSFBBlock: array[0..5, 0..2, 0..3] of
  cardinal = (((6, 5, 5, 5), (9, 9, 9, 9), (6, 9, 9, 9)),
...
begin
  ...
  begin  // Find region boundary for long block case
    ...
    region2Start := sfBandIndex[FSFreq].l[FSI.ch[ch].gr[gr].region0_count +
      FSI.ch[ch].gr[gr].region1_count + 2];  // MI
  end;
...
begin
  ...
  while (sb18 < 576) do 
  begin
    if (gr_info.window_switching_flag <> 0) and (gr_info.mixed_block_flag <> 0) and
      (sb18 < 36) then
      bt := 0
    else
