...
type
  ...
  PPINGSend = ^TPINGSend;
  TPINGSend = object(TObj)
  private
    FSock: PICMPBlockSocket;
    FBuffer: string;
    FSeq: Integer;
    FId: Integer;
    FTimeout: Integer;
  ...
  public
    ...
    property Sock: PICMPBlockSocket read FSock;
  end;

function PingHost(const Host: string): Integer;

function NewPINGSend : PPINGSend;
...
begin
  New(Result,Create);
  Result.FSock := NewICMPBlockSocket();
  Result.FSock.CreateSocket;
  Result.FTimeout := 5000;
  Result.FPacketSize := 32;
  Result.FSeq := 0;
  Randomize;
end;
