...
type
	...
	TFTPProgress = procedure(Sender : PObj; Progress : Integer) of object;
	TFTPStatus = procedure(Sender: PObj; Response: Boolean;
		const Value: string) of object;

	PFTPSend = ^TFTPSend;
	TFTPSend = object(TObj)
	private
		FOnStatus: TFTPStatus;
		FOnProgress : TFTPProgress;
		FSock: PTCPBlockSocket;
		FDSock: PTCPBlockSocket;
		FTimeout: Integer;
		FFTPHost: string;
		FFTPPort: string;
		FResultCode: Integer;
		...
		function DataRead(const DestStream: PStream): Boolean;
		function DataWrite(const SourceStream: PStream): Boolean;
	protected
	...
	public
		...
		property DSock: PTCPBlockSocket read FDSock;
		property DataStream: PStream read FDataStream;
		...
		property OnProgress : TFTPProgress read FOnProgress write FOnProgress;
	end;
...
begin
	New(Result,Create);
	Result.FFullResult := NewStrList;
	Result.FDataStream := NewMemoryStream;
	Result.FSock := NewTCPBlockSocket;
	Result.FDSock := NewTCPBlockSocket;
	Result.DSock.RaiseExcept := true;
	Result.FTimeout := 300000;
	Result.FFTPHost := cLocalhost;
	Result.FFTPPort := cFtpProtocol;
	Result.FUsername := 'anonymous';
	Result.FPassword := 'anonymous@host.com';// + Result.FSock.LocalName;
	Result.FDirectFile := False;
	Result.FPassiveMode := True;
	Result.FForceDefaultPort := False;
	Result.FAccount := '';
	Result.FFWHost := '';
	Result.FFWPort := cFtpProtocol;
	Result.FFWUsername := '';
	Result.FFWPassword := '';
	Result.FFWMode := 0;
end;
...
procedure TFTPSend.DoProgress(Progress: Integer);
begin
	if assigned(OnProgress) then OnProgress(@Self, Progress);
end;
...
begin
	...
	for n := 1 to 4 do
		if n = 1 then
			...
			FDataIP := FDataIP + '.' + Parse(s, ',');
	x := StrToIntDef(Parse(s, ','), 0) * 256;
	x := x + StrToIntDef(Parse(s, ','), 0);
	FDataPort := Int2Str(x);
end;
...
begin
	...
	begin
		...
		FDataPort := Int2Str(FDSock.GetLocalSinPort);
		s := StringReplace(FDataIP, '.', ',');
		s := 'PORT ' + s + ',' + Int2Str(FDSock.GetLocalSinPort div 256)
			+ ',' + Int2Str(FDSock.GetLocalSinPort mod 256);
...
function TFTPSend.DataRead(const DestStream: PStream): Boolean;
var
	x: integer;
	buf: string;
begin
...
function TFTPSend.DataWrite(const SourceStream: PStream): Boolean;
const
	BufSize = 8192;
var
	Bytes: integer;
...
begin
	Result := False;
	FDataStream.Size := 0; //	FDataStream.Clear;
	if Directory <> '' then
		Directory := ' ' + Directory;
...
begin
	...
	FileStream := false;
	if FDirectFile then begin
		if Restore and FileExists(FDirectFileName) then begin
			RetrStream := NewFileStream(FDirectFileName,ofOpenReadWrite or ofShareExclusive);
			FileStream := true;
		end
		else
		begin
			RetrStream := NewFileStream(FDirectFileName,ofOpenWrite or ofCreateNew or ofShareDenyWrite);
			FileStream := true;
		end;
	end
	else
	...
	try
		...
		if Restore then
		begin
			RetrStream.Seek(0, spEnd);
			if FTPCommand('REST ' + Int2Str(RetrStream.Size)) <> 350 then
				Exit;
		end
		else
		if not FileStream then RetrStream.Size := 0;
		if (FTPCommand('RETR ' + FileName) div 100) <> 1 then
			Exit;
		Result := DataRead(RetrStream);
		if not FDirectFile then
			RetrStream.Seek(0, spBegin);
	finally
		if FDirectFile then
			RetrStream.Free;
	end;
...
begin
	...
	if FDirectFile then
		if not FileExists(FDirectFileName) then
			...
			SendStream := NewReadFileStream(FDirectFileName)//,ofOpenRead or ofShareDenyWrite)
	else
	...
	try
		...
		if RestoreAt > StorSize then
			RestoreAt := 0;
		FTPCommand('ALLO ' + Int2Str(StorSize - RestoreAt));
		if FCanResume then
			if FTPCommand('REST ' + Int2Str(RestoreAt)) <> 350 then
				Exit;
		SendStream.Seek(RestoreAt, spBegin);
...
begin
	...
	try
		...
		s := 'PORT ' + s + ',' + Int2Str(StrToIntDef(FromFTP.DataPort, 0) div 256)
			+ ',' + Int2Str(StrToIntDef(FromFTP.DataPort, 0) mod 256);
