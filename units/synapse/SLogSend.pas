...
type
  ...
  PSyslogSend = ^ TSyslogSend;
  TSyslogSend = object(TObj)
  private
    FSyslogHost: string;
    FSyslogPort: string;
...
function NewSyslogSend : PSyslogSend;
...
begin
  ...
  Result.FSyslogHost := cLocalhost;
  Result.FSyslogPort := cSysLogProtocol;
  Result.FFacility := FCL_Local0;
  Result.FSeverity := Debug;
  Result.FTag := ExtractFileName(ParamStr(0));
  Result.FMessage := '';
end;
