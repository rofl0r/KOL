...
function NewPOP3Send : PPOP3Send;
...
begin
  ...
  Result.FPOP3host := cLocalhost;
  Result.FPOP3Port := cPop3Protocol;
  Result.FUsername := '';
  Result.FPassword := '';
  Result.FStatCount := 0;
  Result.FStatSize := 0;
  Result.FAuthType := POP3AuthAll;
end;
...
begin
  if Value = 0 then
    ...
    FSock.SendString('UIDL ' + Int2Str(Value) + CRLF);
  Result := ReadResult(Value = 0) = 1;
end;
