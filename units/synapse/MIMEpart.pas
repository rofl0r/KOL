...
uses
  KOL,SynaChar, SynaCode, SynaUtil, MIMEinLn;
...
type
  ...
  private
    ...
    FFileName: string;
    FLines: PStrList;
    FDecodedLines: PStream;//TMemoryStream;
  ...
  public
    ...
    procedure Clear;
    function ExtractPart(Value: PStrList; BeginLine: Integer): Integer;
    ...
    property DecodedLines: PStream read FDecodedLines;
  end;
...
function NormalizeHeader(Value: PStrList; var Index: Integer): string;
function GenerateBoundary: string;

function NewMimePart : PMimePart;
...
begin
  ...
  Result.FDecodedLines := NewMemoryStream;//TMemoryStream.Create;
  Result.FTargetCharset := GetCurCP;
end;
...
begin
  ...
  FLines.Clear;
//  FDecodedLines.Clear;
  FDecodedLines.Size := 0;
end;

{==============================================================================}

function TMIMEPart.ExtractPart(Value: PStrList; BeginLine: Integer): Integer;
var
  n, x, x1, x2: Integer;
  t: PStrList;
  s, su, b: string;
  ...
  fn: string;
begin
  t := NewStrList;//TStringlist.Create;
  try
    { defaults }
    FLines.Clear;
    Primary := 'text';
...
var
  ...
  s: string;
begin
  FDecodedLines.Size := 0;//  FDecodedLines.Clear;
  ...
  try
    case FPrimaryCode of
      MP_MULTIPART, MP_MESSAGE:
        FLines.LoadFromStream(FDecodedLines,false);
      MP_TEXT, MP_BINARY:
        ...
        begin
          l.LoadFromStream(FDecodedLines,false);
...
begin
  Randomize;
  x := Random(MaxInt);
  Result := '--' + Int2Hex(x, 8) + '_Synapse_message_boundary--';
end;
