...
function Rfc822DateTime(t: TDateTime): string;
begin
								Result := Date2StrFmt('ddd, d MMMM yyyy ',t) + Time2StrFmt('hh:mm:ss', t);
...
function CDateTime(t: TDateTime): string;
begin
								Result := Date2StrFmt('mmm dd ',t) + Time2StrFmt('hh:mm:ss', t);
								if Result[5] = '0' then
												Result[5] := ' ';
end;
...
begin
  Result := '';
  for n := 1 to Length(Value) do
    Result := Result + Int2Hex(Byte(Value[n]), 2);
...
function StrToIntDef(const Value : string; Default : Integer) : Integer;
begin
        if Value[1]= '$' then Result := Hex2Int(Value)
        else
                Result := Str2Int(Value);
        if (Result = 0) and ((Value <> '0') and (Value <> '$0')) then Result := Default;
end;
