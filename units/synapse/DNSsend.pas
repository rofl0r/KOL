...
function GetMailServers(const DNSHost, Domain: string;
	const Servers: PStrList): Boolean;

function NewDNSSend : PDNSSend;
...
begin
	...
	begin
		case RType of
			QTYPE_A:
				begin
					Result := Int2Str(Ord(FBuffer[j]));
					Inc(j);
					Result := Result + '.' + Int2Str(Ord(FBuffer[j]));
					Inc(j);
					Result := Result + '.' + Int2Str(Ord(FBuffer[j]));
					Inc(j);
					Result := Result + '.' + Int2Str(Ord(FBuffer[j]));
