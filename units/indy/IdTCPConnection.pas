...
begin
  ...
  begin
    LFileStream := NewFileStream(AFile, fmOpenRead or fmShareDenyNone {???} or ofOpenAlways {???});//TFileStream.Create(AFile, fmOpenRead or fmShareDenyNone);
    try
...
begin
  ...
  begin
    WriteLn(StringReplace(axHeader.Items[i], '=', ': ', []));
  end;
  WriteLn('');
end;
