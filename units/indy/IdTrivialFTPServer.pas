...
type
  ...
  TIdTrivialFTPServer = object(TIdUDPServer)
  protected
  ...
  protected
    ...
    procedure DoUDPRead(AData: TStream; ABinding: TIdSocketHandle); virtual;//override;
  public
...
//  published
  ...
  PIdTrivialFTPServer=^TIdTrivialFTPServer;
  function NewIdTrivialFTPServer(axOwner: PControl):PIdTrivialFTPServer;

type

  TIdTFTPServerThread = object(TThread)
  private
    FStream: TStream;
...
//    procedure Execute; virtual;//override;
  ...
  PIdTFTPServerThread=^TIdTFTPServerThread;
  function NewIdTFTPServerThread (AnOwner: TIdTrivialFTPServer; const Mode: TIdTFTPMode;
      const PeerInfo: TPeerInfo;
      AStream: TStream; const FreeStreamOnTerminate: boolean; const
        RequestedBlockSize: Integer = 0):PIdTFTPServerThread;

//  type
{TIdTFTPServerSendFileThread = object(TIdTFTPServerThread)
  end;
  TIdTFTPServerReceiveFileThread = object(TIdTFTPServerThread)
  end;}
...
function NewIdTFTPServerThread (AnOwner: TIdTrivialFTPServer;
  const Mode: TIdTFTPMode; const PeerInfo: TPeerInfo;
  AStream: TStream; const FreeStreamOnTerminate: boolean; const
    RequestedBlockSize: Integer):PIdTFTPServerThread;
begin
New( Result, Create );
with Result^ do
begin
