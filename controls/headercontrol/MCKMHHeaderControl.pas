...
uses
    Windows, Controls, Classes, KOLMHHeaderControl,mirror,KOL,Graphics;
...
type
  ...
  TKOLMHHeaderControl = class(TKOLControl)
  private
...
constructor TKOLMHHeaderControl.Create(AOwner: TComponent);
var
  i:Integer;
begin
  inherited;
...
destructor TKOLMHHeaderControl.Destroy;
var
  i:Integer;
begin
  For i:=0 to 3 do
...
procedure TKOLMHHeaderControl.Paint;
var
  R:TRect;
  dsel:Integer;
  dv:Integer;
...
procedure TKOLMHHeaderControl.SetupConstruct(SL: TStringList; const AName,
  AParent, Prefix: String);
var S: String;
begin
  S := GenerateTransparentInits;
  SL.Add( Prefix + AName + ' := PMHHeaderControl( New' + TypeName + '( '
...
function TKOLMHHeaderControl.SetupParams(const AName,AParent : String) : String;
{const
...
(*procedure TKOLMHHeaderControl.AssignEvents(SL: TStringList; const AName: String);
begin
  inherited;
...
function TKOLMHHeaderControl.AdditionalUnits;
begin
   Result := ', KOLMHHeaderControl';
end;
...
procedure TKOLMHHeaderControl.SetupFirst(SL: TStringList; const AName,
  AParent, Prefix: String);
var
  i:Byte;
begin
...
//    SL.Add( Prefix + AName + '.CalColors.TitleTextColor:='+Color2Str(CalColors.TitleTextColor)+';');
//  SL.Add('');
//  SL.Add(Prefix+AName+':=NewMHHeaderControl('+AParent+');');
//  SL.Add(Prefix+AName+':=NewMHHeaderControl('+AParent+','+Int2Str(Left)+', '+Int2Str(Top)+', '+Int2Str(Width)+', '+Int2Str(Height)+');');
 { For i:=0 to 3 do
  begin
...
procedure TKOLMHHeaderControl.SetupLast(SL: TStringList; const AName,
  AParent, Prefix: String);
var
  i:Integer;  
begin
...
function TKOLMHHeaderControl.GetField(const Index:Integer):TField;
begin
  Result:=FFields[Index];
end;

procedure TKOLMHHeaderControl.SetField(const Index:Integer; const Value:TField);
begin
  FFields[Index]:=Value;
  Change;
end;
...
procedure Register;
begin
  RegisterComponents('KOL WIN32', [TKOLMHHeaderControl]);
end;
