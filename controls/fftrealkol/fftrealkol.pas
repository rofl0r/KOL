...
______________________________________________________________________________
        Translation to KOL ObjectPascal by :
          Thaddy de Koning
          thaddy@thaddy.com

          Compared to the default implementation by Frederic
          my performance test consistanly showed an additional 4 mono channels
          at 44100HZ (AMD 1000) 47 without system.dcu replacement and 46 with
          system.dcu replacement, (43 original code with all optimizations ON)
          Which is significant.
          *Only* changed Class to Tobj and replaced constructors with
          global constructing functions. Rest of the code is untouched.
...
type
    ...
    PBitReversedLUT = ^TBitReversedLUT;
    TBitReversedLUT = object(Tobj)
    private
      _ptr : plongint;
    public
    ...
    PFFTReal = ^TFFTReal;
    TFFTReal = object(Tobj)
    private
      _bit_rev_lut  : pBitReversedLUT;
      _trigo_lut    : pTrigoLUT;
      _sqrt2_2      : flt_t;
      _length       : longint;
      _nbr_bits     : integer;
      _buffer_ptr   : pflt_t;
    ...
    function NewFFTReal(const length:longint):PFFTReal;
    function NewTrigoLUT(const nbr_bits:integer):PTrigoLUT;
    function NewBitReversedLUT(const nbr_bits:integer):PBitReversedLUT;
...
begin
  ...
  begin
    ...
    _bit_rev_lut := NewBitReversedLUT(Floor(Ln(length) / Ln(2) + 0.5));
    _trigo_lut := NewTrigoLUT(Floor(Ln(length) / Ln(2) + 0.05));
    _sqrt2_2 := Sqrt(2) * 0.5;
    _buffer_ptr := nil;
    if _nbr_bits > 2 then
